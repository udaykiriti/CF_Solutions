...
template <typename T>
using pq = priority_queue<T>; // high prec., pq<T> p;
template <typename T>
using pq_ = priority_queue<T, vector<T>, greater<T>>; // dec. as: pq_<T> x;
template <typename T1, typename T2>
using vp = vector<pair<T1, T2>>; // vp<T1, T2> ..
...
i64 Xuclid(i64 a, i64 b, i64 &x, i64 &y)
{
  if (b == 0)
  {
    x = 1;
...
template <typename T>
T LOG(T base, T power)
{
  assert(power > 0 && base > 1);
  T dg = 0;
  while (power >= base)
  {
    ++dg;
